...
while (model.generation < model.max_generation) &&...
  ...
  % update population
  model = model.update_P(children, model);
  
  clf;
  show_P_costs(1, model.P_costs, model.optimal, 'og');

  % update best individuals
  model = model.update_best(model);
  
  % individual selection for new generation
  model = model.i_selection(model);
